#!/usr/bin/env bash
# The 20-s Civ1 game film 'J. S. Bachs Kathedrale' (wunderfilm_13_bach):
...
PY="${PYTHON:-python}"
BUILD="${BUILD:-$HERE/build/b20}"
NAME="${NAME:-wunderfilm_13_bach}"
...
"$PY" -c "import sys; sys.path.insert(0, '$HERE'); import film_bach20 as F; F.build((64, 36))" > /dev/null

# render: two processes with two threads each (even / odd frames)
"$PY" "$HERE/render.py" --film bach20 --out "$BUILD" --no-lines --threads 2 \
...
"$PY" "$HERE/stylize.py" --inp "$BUILD" --out "$BUILD/png" --look paint --frames none --film bach20
for k in 0 1 2 3; do
  "$PY" "$HERE/stylize.py" --inp "$BUILD" --out "$BUILD/png" --look paint --no-titles --fade-in 0.35 \
    --film bach20 --frames "$(seq -s, $k 4 479)" > "$BUILD/stylize_$k.log" &
done
wait
...
"$PY" "$HERE/audio_bach20.py" --out "$BUILD/soundtrack.wav"
