#!/usr/bin/env bash
# The 20-s Civ1 game film 'Magellans Expedition' (wunderfilm_08_magellan):
...
PY="${PYTHON:-python}"
BUILD="${BUILD:-$HERE/build/m20}"
NAME="${NAME:-wunderfilm_08_magellan}"
OUTDIR="${OUTDIR:-$HERE/game_films}"
mkdir -p "$BUILD/png" "$BUILD/png_stab" "$OUTDIR"

# the photographed day sky (CC0, ~20 MB, not in git); nothing is sculpted, the build is a warm-up
...
"$PY" -c "import sys; sys.path.insert(0, '$HERE'); import film_magellan20 as F; F.build((64, 36))" > /dev/null

# render: two processes with two threads each (even / odd frames)
"$PY" "$HERE/render.py" --film magellan20 --out "$BUILD" --no-lines --threads 2 \
...
"$PY" "$HERE/stylize.py" --inp "$BUILD" --out "$BUILD/png" --look paint --frames none --film magellan20
for k in 0 1 2 3; do
  "$PY" "$HERE/stylize.py" --inp "$BUILD" --out "$BUILD/png" --look paint --no-titles --fade-in 0.35 \
    --film magellan20 --frames "$(seq -s, $k 4 479)" > "$BUILD/stylize_$k.log" &
done
wait
...
"$PY" "$HERE/audio_magellan20.py" --out "$BUILD/soundtrack.wav"
