#!/usr/bin/env bash
# The 20-s Civ1 game film 'Kopernikus' Observatorium' (wunderfilm_10_copernicus):
...
PY="${PYTHON:-python}"
BUILD="${BUILD:-$HERE/build/k20}"
NAME="${NAME:-wunderfilm_10_copernicus}"
OUTDIR="${OUTDIR:-$HERE/game_films}"
mkdir -p "$BUILD/png" "$BUILD/png_stab" "$OUTDIR"

# the photographed day sky (CC0, ~20 MB, not in git); the figures are sculpted and cached
# once here, before two render processes would race to write the same cache files
ls "$HERE"/assets/hdri/*_nosun.hdr > /dev/null 2>&1 || "$PY" "$HERE/fetch_hdri.py"
"$PY" -c "import sys; sys.path.insert(0, '$HERE'); import film_copernicus20 as F; F.build((64, 36))" > /dev/null

# render: two processes with two threads each (even / odd frames)
"$PY" "$HERE/render.py" --film copernicus20 --out "$BUILD" --no-lines --threads 2 \
...
"$PY" "$HERE/stylize.py" --inp "$BUILD" --out "$BUILD/png" --look paint --frames none --film copernicus20
for k in 0 1 2 3; do
  "$PY" "$HERE/stylize.py" --inp "$BUILD" --out "$BUILD/png" --look paint --no-titles --fade-in 0.35 \
    --film copernicus20 --frames "$(seq -s, $k 4 479)" > "$BUILD/stylize_$k.log" &
done
wait
...
"$PY" "$HERE/audio_copernicus20.py" --out "$BUILD/soundtrack.wav"
