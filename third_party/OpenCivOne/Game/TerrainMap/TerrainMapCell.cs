...
namespace OpenCivOne
{
	...
	public class TerrainMapCell : IEquatable<TerrainMapCell>
	{
		...
		// Temporary function to avoid error
		...
		/*public string TerrainName
		{
			get
			{
				if (this.oTerrainDefinition == null)
				{
					...
					{
						this.oTerrainDefinition = this.oParent.Parent.GameData.TerrainTypes.GetValueByKey(this.eTerrainType);
					}
					else
					{
						return "";
		...
		public int TotalCellWorth
		{
			get
			{
				if (this.oParent != null && this.oParent.Parent != null && this.eTerrainType != TerrainTypeEnum.Water && this.iTotalCellWorth <= 0)
				{
					...
					{
						int cellWorth = 0;
						GPoint newPos = this.oPosition.Offset(this.oParent.Parent.GameData.CityOffsets[k]);
