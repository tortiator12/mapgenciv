...
namespace OpenCivOne
{
	public class StartGameMenu
	{
		...
		internal void ResetProject1991StateForNewGame()
		{
			this.parent.UnitAutomationState.Clear();
			this.parent.ResetTransientRuntimeStateForNewGame();
			this.parent.CityWorker.ResetTransientStateForNewGame();
			this.parent.GameData.ResetMutableStateForNewGame();
			Player barbarians =
				this.parent.GameData.Players[0];
			barbarians.NationalityID = 0;
			barbarians.Name =
				this.parent.GameData.Nations[0].Leader;
			barbarians.Nation =
				this.parent.GameData.Nations[0].Nation;
			barbarians.Nationality =
				this.parent.GameData.Nations[0].Nationality;
		}
		...
		public void F5_0000_0000_InitNewGameData()
		{
			...
			this.parent.GameData.DifficultyLevel = (short)this.parent.MenuBoxDialog.F0_2d05_0031_ShowMenuBox(
				ClassicGameText.Current[ClassicGameTextKey.DifficultyMenu], 160, 35, false, false, true);
			...
			if (this.parent.Var_2fa2_DialogMousePressed)
			{
				...
				if (mouseEvent.Position.X < 135)
				{
					...
					this.parent.GameData.DifficultyLevel = (short)this.parent.Tools.F0_2dc4_007c_CheckValueRange((mouseEvent.Position.Y - 12) / 35, 0, 4);
				}
			}
			...
			competitionList.Append(ClassicGameText.Current[ClassicGameTextKey.CompetitionTitle]);

			for (int i = 7; i > 2; i--)
			{
				// Instruction address 0x0000:0x016b, size: 5
				competitionList.Append(ClassicGameText.Current.Format(ClassicGameTextKey.CivilizationCount, i));
			}
			...
			this.parent.GameData.AIOpponentCount = (short)this.parent.Tools.F0_2dc4_007c_CheckValueRange(6 - this.parent.GameData.AIOpponentCount, 2, 6);
			...
			for (int i = 0; i < 256; i++)
			{
				this.parent.GameData.CityPositions[i] = OpenCivOneGame.InvalidPosition;
			}
			...
			tribeList.Append(ClassicGameText.Current[ClassicGameTextKey.PickTribe]);
			...
			this.parent.GameData.NextPlayerHistoryRankingTurn += (short)this.parent.CAPI.RNG.Next(50);
			...
			startText.Append(ClassicGameText.Current[ClassicGameTextKey.InitialRoadsSuffix]);
		...
		internal void SelectAiProfileForNewGame()
		{
			this.parent.Var_2f9a_MenuBoxDefaultOptionIndex = 0;
			int selectedProfile;
			do
			{
				selectedProfile =
					this.parent.MenuBoxDialog
						.F0_2d05_0031_ShowMenuBox(
							ClassicGameText.Current[
								ClassicGameTextKey.AiProfileMenu],
							160,
							35,
							false,
							false,
			...
			// The profile selector and the following difficulty selector use
			// the same panel. Restore the clean new-game backdrop before the
			// next menu is drawn so both menus cannot overlap.
		...
		internal void ApplySelectedProfileDefaults()
		{
			if (ClassicAiRuntimeProfiles.UsesSmartEnhancements(
					this.parent.GameData.AiProfile))
			{
				this.parent.GameData.GameSettingFlags
					.Enable1991PlusDefaults();
				this.parent.GameData.GameSettingFlags.Animations = false;
				this.parent.GameData.GameSettingFlags.BuildPalace = false;
				return;
			}
		...
		/// <returns></returns>
		public bool F5_0000_07c7_InitPlayerData(int playerID)
		{
			...
			if (playerID != this.parent.GameData.HumanPlayerID || this.parent.GameData.TurnCount == 0)
			{
				...
				this.parent.GameData.Players[playerID].HistoryRankingScore = 0;
				...
				L0a5e:
					...
					int distance;
					int cityID = this.parent.Tools.F0_2dc4_0102_FindNearestCity(xStart, yStart);
					...
					else
					{
						...
						distance = this.parent.Tools.F0_2dc4_0289_GetShortestDistance(xStart, yStart,
					...
					if (this.parent.GameData.TurnCount == 0)
					{
						for (int j = 1; j < playerID; j++)
						{
							...
							int newDistance = this.parent.Tools.F0_2dc4_0289_GetShortestDistance(xStart, yStart,
					...
					if (tryCount <= 2000 || (playerID == this.parent.GameData.HumanPlayerID && this.parent.GameData.TurnCount == 0))
					{
						...
						if (this.parent.MapManagement.F0_2aea_14e0_GetCellActiveUnitPlayerID(xStart, yStart) != -1 ||
							(12 - (tryCount / 32)) > buildScore || distance < (10 - (tryCount / 64)) ||
					...
					for (int i = 0; i < 128; i++)
					{
						if (this.parent.GameData.Players[0].Units[i].UnitType != UnitTypeEnum.None &&
							this.parent.Tools.F0_2dc4_0289_GetShortestDistance(xStart, yStart,
		...
		/// <summary>
		/// This function tests if an AI player has no more cities or settlers.
		/// If both counts are zero the player is marked as destroyed and all units that belong to the AI player are deleted.
		/// </summary>
		/// <param name="playerID">AI player to test</param>
		/// <param name="playerID1">The player that destroyed the last city</param>
		public void F5_0000_0e6c_TestIfAIPlayerIsDestroyed(int playerID, int playerID1)
		{
			...
			if (playerID != 0 && playerID != this.parent.GameData.HumanPlayerID)
			{
				...
				if (cityCount == 0 && !hasSettler)
				{
					...
					this.parent.Segment_2459.F0_2459_05ee_SpaceshipArrivalFailed(playerID);
		...
		public void F5_0000_1af6_LoadGovernmentImage()
		{
			...
			// function body
			GBitmap? preservedDiplomacyFrame = null;
			if (this.parent.Var_3934 != -1)
			{
				// Screen 1 is the restore buffer for the active diplomacy
				// scene, but government pictures also use it as scratch space.
				GBitmap diplomacyFrame =
					this.parent.Graphics.Screens.GetValueByKey(1);
				preservedDiplomacyFrame =
					new GBitmap(diplomacyFrame.Width, diplomacyFrame.Height);

				for (int y = 0; y < diplomacyFrame.Height; y++)
				{
					for (int x = 0; x < diplomacyFrame.Width; x++)
					{
						preservedDiplomacyFrame.SetPixel(
							x,
							y,
							diplomacyFrame.GetPixel(x, y));
					}
				}
			}
			...
			if (preservedDiplomacyFrame != null)
			{
				this.parent.Graphics.DrawBitmapToScreen(
					preservedDiplomacyFrame,
					0,
					0,
					preservedDiplomacyFrame.Width,
					preservedDiplomacyFrame.Height,
					this.parent.Var_19d4_Screen1_Rectangle,
					0,
					0);
			}
