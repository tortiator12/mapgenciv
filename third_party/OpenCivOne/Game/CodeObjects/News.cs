namespace OpenCivOne
{
	public class News
	{
		...
		public void F21_0000_0000_ShowNews(int cityID, string newsText)
		{
			...
			this.parent.NewsShownForTests?.Invoke(cityID, newsText);
			...
			if (cityID < 0)
			{
				for (int i = 0; i < 128; i++)
				{
					...
					{
						if (cityID == -1 || this.parent.GameData.Cities[i].HasImprovement(ImprovementEnum.Palace))
						{
							newCityID = i;
						}
					}
			...
			this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0($"January 1, {this.parent.Segment_1238.GetYearAsString(this.parent.GameData.Year)}", 8, 28, 0);
