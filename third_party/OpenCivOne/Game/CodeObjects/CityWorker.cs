...
namespace OpenCivOne
{
	...
	{
		internal const int QolStatusX = 98;
		internal const int QolStatusWidth = 126;
		internal const int ClassicProductionPanelX = 230;
		internal const int MeasurementOnlyFlag = -1;

		internal readonly record struct ScratchState(
			int Var2494,
			int Var2496,
			int CityID,
			int CityOffsetCount,
		...
		internal ScratchState CaptureScratchState() =>
			new(
				this.Var_2494,
				this.Var_2496,
				this.Var_653e_CityID,
				this.Var_6540_CityOffsetCount,
				this.Var_6542,
				this.Var_6546,
		...
		internal void RestoreScratchState(ScratchState state)
		{
			this.Var_2494 = state.Var2494;
			this.Var_2496 = state.Var2496;
			this.Var_653e_CityID = state.CityID;
			this.Var_6540_CityOffsetCount = state.CityOffsetCount;
			this.Var_6542 = state.Var6542;
			this.Var_6546 = state.Var6546;
			this.Var_6548_PlayerID = state.PlayerID;
			this.Var_6b30 = state.Var6b30;
			this.Var_70e8 = state.Var70e8;
		}

		internal void ResetTransientStateForNewGame()
		{
			this.Var_2494 = 0;
			this.Var_2496 = 0;
			this.Var_653e_CityID = 0;
			this.Var_6540_CityOffsetCount =
				this.parent.CityOffsets.Length;
			this.Var_6542 = 0;
			this.Var_6546 = 0;
			this.Var_6548_PlayerID = 0;
			this.Var_6b30 = 0;
			this.Var_70e8 = 0;
		}

		private string GetLocalizedImprovementTypeName(int typeID)
		{
			string originalName =
				this.parent.GameData.GetImprovementType(typeID).Name;
			return ClassicDisplayNames.ImprovementOrWonder(
				typeID,
				originalName);
		}

		private void Draw1991PlusCityIntelligence(int cityID)
		{
			if (this.Var_2496 != 0 ||
				this.Var_6548_PlayerID !=
			...
			string netText =
				ClassicGameText.Current.Format(
					ClassicGameTextKey.CityQolNetSummary,
					FormatSigned(snapshot.NetFood),
					FormatSigned(snapshot.NetProduction),
					FormatSigned(snapshot.NetTrade));
			...
			string populationText =
				snapshot.PopulationTrend switch
				{
					ClassicPopulationTrend.Growth
						when snapshot.PopulationTurns.HasValue =>
						ClassicGameText.Current.Format(
							ClassicGameTextKey
								.CityQolGrowthTurns,
							snapshot.PopulationTurns.Value),
					ClassicPopulationTrend.Famine
						when snapshot.PopulationTurns.HasValue =>
						ClassicGameText.Current.Format(
							ClassicGameTextKey
								.CityQolFamineTurns,
							snapshot.PopulationTurns.Value),
					_ => ClassicGameText.Current[
						ClassicGameTextKey
							.CityQolStableGrowth]
				};
			this.parent.DrawTools
			...
			if (snapshot.NextTurnShieldOverflow > 0)
			{
				productionText += " " +
					ClassicGameText.Current.Format(
						ClassicGameTextKey
							.CityQolShieldOverflow,
						snapshot.NextTurnShieldOverflow);
			}

			List<string> warnings = [];
			if (snapshot.WillStarveNextTurn)
			{
				warnings.Add(
					ClassicGameText.Current[
						ClassicGameTextKey
			...
			if (snapshot.WillEnterDisorderNextTurn ||
				snapshot.IsInDisorder)
			{
				warnings.Add(
					ClassicGameText.Current[
						ClassicGameTextKey
			...
			string statusText = warnings.Count == 0
				? productionText
				: $"{productionText} {string.Join(" ", warnings)}";
			this.parent.DrawTools
				.F0_1182_005c_DrawStringToScreen0(
					this.parent.LanguageTools
						.F0_2f4d_04f7_TrimStringToWidth(
					...
					(byte)(
						warnings.Count > 0 ||
							snapshot.ProductionStalled ||
							snapshot.NextTurnShieldOverflow > 0
								? 12
								: 15));
		}
		...
		private string GetLocalizedProductionName(int productionID)
		{
			return productionID >= 0
				? ClassicDisplayNames.Unit(
					...
					this.parent.GameData.Units[productionID].Name)
				: this.GetLocalizedImprovementTypeName(-productionID);
		}

		private string GetLocalizedNationality(int playerID)
		{
			Player player = this.parent.GameData.Players[playerID];
			int nationalityID = player.NationalityID;
			if (nationalityID < 0 ||
				nationalityID >= this.parent.GameData.Nations.Length)
			{
			...
			return ClassicDisplayNames.Nation(
				nationalityID,
				player.Nationality,
				this.parent.GameData.Nations[nationalityID].Nationality);
		}
		...
		internal T SynchronizeProcessing<T>(Func<T> operation)
		{
			ArgumentNullException.ThrowIfNull(operation);
			lock (this.processingGate)
			{
				return operation();
			}
		}
		...
		public int F0_1d12_0045_ProcessCityState(int cityID, int flag) =>
			this.SynchronizeProcessing(
				() => this.ProcessCityStateCore(cityID, flag));

		private int ProcessCityStateCore(int cityID, int flag)
		{
			...
			int local_108 = 0;
			int local_10a = 0;
			int project1991NewSupportedUnitGraceID = -1;
			int project1991SupportSizeGrace = 0;
			...
			if (city.StatusFlag != 0xff)
			{
				...
				for (int i = 0; i < 128; i++)
				{
					if (this.parent.GameData.Cities[i].StatusFlag != 0xff && i != city.ID)
					{
						...
						local_b0 = this.parent.Tools.F0_2dc4_0289_GetShortestDistance(
							city.Position.X,
							city.Position.Y,
			...
			L045f:
				...
				if (flag == 1)
				{
					...
					this.parent.DrawTools.F0_1182_00b3_DrawCenteredStringToScreen0(
						ClassicGameText.Current.Format(
							ClassicGameTextKey.CityPopulation,
							this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID),
							this.parent.Tools.F0_2dc4_0337_PopulationValueToString(local_e8)),
						104, 2, 15);
					...
					DrawCityNeighborhood(city, Arr_a6);
				}
					...
					if (city.FoodCount < 0)
					{
						...
						if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
						{
							...
							this.parent.News.F21_0000_0000_ShowNews(cityID,
								ClassicGameText.Current.Format(
									local_e8 != -1
										? ClassicGameTextKey.CityFoodStorageExhaustedSettlerLost
										: ClassicGameTextKey.CityFoodStorageExhaustedFamineFeared,
									this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
						...
						else
						{
							...
							if (city.ActualSize <= 0)
							{
								...
								this.parent.StartGameMenu.F5_0000_0e6c_TestIfAIPlayerIsDestroyed(this.Var_6548_PlayerID, 0);
						...
						this.parent.PlayerTurn.F0_1403_3ed7(local_d8, local_e4);
					}
						...
						if (city.ActualSize > 10 && !city.HasImprovement(ImprovementEnum.Aqueduct) && (this.parent.GameData.DebugFlags & 0x4) != 0)
						{
							...
							if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
							{
								...
								this.parent.Segment_1238.F0_1238_001e_ShowDialog(
									ClassicGameText.Current.Format(
										ClassicGameTextKey.CityRequiresAqueduct,
										this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)),
									100,
									80);
							}
				...
				for (int i = 0; i < this.Var_6540_CityOffsetCount; i++)
				{
					...
					if (this.Var_6548_PlayerID != 0 &&
						(this.parent.GameData.MapVisibility[
							this.parent.MapManagement.AdjustXPosition(local_c6),
							local_d2] & (1 << this.Var_6548_PlayerID)) == 0)
					{
						Arr_3e[i] = 1;
					}

					// Instruction address 0x1d12:0x0bf4, size: 5
					local_e8 = this.parent.MapManagement.F0_2aea_14e0_GetCellActiveUnitPlayerID(local_c6, local_d2);
					...
					(city.StatusFlag & 0x1) != 0 || this.Var_70e8 != 0)
				{
					...
					else
					{
						if (flag == 0 && this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
						{
							...
							this.parent.Segment_1238.F0_1238_001e_ShowDialog(
								ClassicGameText.Current.Format(
									ClassicGameTextKey.CityPopulationDecrease,
									this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)),
								100,
								80);
				...
				while (local_50 > this.parent.Var_e8b8)
				{
					...
					for (int i = 0; i < this.Var_6540_CityOffsetCount; i++)
					{
						if (Arr_3e[i] == 0)
						{
							...
							local_d4 = local_4 * (16 / this.parent.Tools.F0_2dc4_007c_CheckValueRange(
							...
							local_d4 += ((city.ActualSize * 3) / this.parent.Tools.F0_2dc4_007c_CheckValueRange(
							...
							local_d4 += ((city.ActualSize * 2) / this.parent.Tools.F0_2dc4_007c_CheckValueRange(this.parent.Var_70da_Arr[2], 1, 99)) * local_46;
			...
			L12c2:
				...
				local_ac = this.parent.Tools.F0_2dc4_007c_CheckValueRange(local_ac, 0, local_f8);
				...
				if (flag == 0)
				{
					...
					if (city.CurrentProductionID >= 0)
					{
						...
						{
							if (this.Var_6548_PlayerID != this.parent.GameData.HumanPlayerID &&
								city.CurrentProductionID == (int)UnitTypeEnum.Caravan &&
								!this.parent.AIEngine.TryFindClassicAiCaravanDestination(
									this.Var_6548_PlayerID,
									out _))
							{
							...
							if (this.Var_6548_PlayerID != this.parent.GameData.HumanPlayerID &&
								city.CurrentProductionID == (int)UnitTypeEnum.Diplomat &&
								!this.parent.UnitManagement.HasFreeActiveUnitSlot(
									this.Var_6548_PlayerID))
							{
								goto L2307;
							}
							...
							else
							{
								...
								if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID || city.CurrentProductionID != (int)UnitTypeEnum.Diplomat)
								{
									...
									if (this.Var_6548_PlayerID ==
										...
										city.CurrentProductionID !=
											(int)UnitTypeEnum.Diplomat &&
										city.CurrentProductionID !=
											(int)UnitTypeEnum.Caravan)
									{
										// The turn-level reassignment pass ran
										// before production. Give this one new
										// supported unit until the next pass
										// instead of disbanding it immediately.
										project1991NewSupportedUnitGraceID =
											local_ba;
									}
								}
								...
								{
									this.parent.GameData.Players[this.Var_6548_PlayerID].Units[local_ba].Status |= UnitStatusEnum.Veteran;
								}
								...
								if (local_ba != -1 && city.CurrentProductionID == 0 && (city.ActualSize > 1 || this.parent.GameData.Players[this.Var_6548_PlayerID].CityCount > 1))
								{
									...
									if (this.Var_6548_PlayerID ==
										...
										this.parent.GameData.GameSettingFlags
											.AutomaticHomeCityReassignment)
									{
										// The pre-city pass could not know
										// whether this Settler would finish.
										// Keep the lost population support
										// slot until the next pass.
										project1991SupportSizeGrace = 1;
									}
								...
								if (this.Var_6548_PlayerID != this.parent.GameData.HumanPlayerID && local_ba != -1)
								{
									...
									{
										if (this.parent.AIEngine.TryFindClassicAiCaravanDestination(
											this.Var_6548_PlayerID,
											out local_108))
										{
											// Instruction address 0x1d12:0x180e, size: 5
											this.parent.Segment_2459.F0_2459_0948_CaravanArrivesAtDestinationCity(this.Var_6548_PlayerID, local_ba, local_108);
										}
									}
									...
									if (city.CurrentProductionID == 26)
									{
										...
										for (int i = 0; i < 128; i++)
										{
											if (this.parent.GameData.Cities[i].PlayerID != this.parent.GameData.HumanPlayerID || this.parent.GameData.Cities[i].StatusFlag != 0xff)
											{
												...
												local_e8 = this.parent.Tools.F0_2dc4_0177_FindNearestPlayerUnit(this.Var_6548_PlayerID, local_ba,
													this.parent.GameData.Cities[i].Position.X,
													this.parent.GameData.Cities[i].Position.Y);

												int unitDistance = Math.Abs(this.parent.Tools.F0_2dc4_0289_GetShortestDistance(this.parent.GameData.Cities[i].Position, 
										...
										else
										{
											...
											if (local_ba != -1)
											{
												this.parent.GameData.Players[this.Var_6548_PlayerID].Units[local_ba].GoToDestination = this.parent.GameData.Cities[local_b0].Position;
											}
										}
									}
								...
								else
								{
									...
									if (local_ba != -1 && (city.CurrentProductionID == 0 || city.CurrentProductionID >= 26))
									{
										...
										this.parent.Segment_1238.F0_1238_001e_ShowDialog(
											ClassicGameText.Current.Format(
												ClassicGameTextKey.CityBuildsUnit,
												this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID),
												ClassicDisplayNames.Unit(
													(UnitTypeEnum)city.CurrentProductionID,
							...
							if (local_e8 != -1)
							{
								if (local_e8 == 41)
								{
									...
									this.parent.News.F21_0000_0000_ShowNews(-1,
										ClassicGameText.Current.Format(
											ClassicGameTextKey.CityNuclearWeaponsTestNear,
											this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
								...
								if ((this.parent.GameData.PlayerFlags & (1 << this.Var_6548_PlayerID)) != 0 || local_e8 > 24)
								{
										...
										(local_e8 <= 21 || local_e8 > 24) && (city.StatusFlag & 0x10) == 0 && local_e8 != 1)
									{
										this.parent.CityView.F19_0000_0000_ShowCityLayout(cityID, local_e8,
											ClassicGameText.Current.Format(
												ClassicGameTextKey.CityBuildsImprovement,
												this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID),
												this.GetLocalizedImprovementTypeName(local_e8)));
									}
									else
									{
										this.parent.News.F21_0000_0000_ShowNews(cityID,
								...
								if (local_e8 == 1)
								{
										...
										this.parent.GameData.Players[this.parent.GameData.HumanPlayerID].Diplomacy[this.Var_6548_PlayerID].HasFlag(DiplomacyFlagsEnum.Unknown40))
									{
										...
										if (this.Var_6548_PlayerID != this.parent.GameData.HumanPlayerID)
										{
											...
											this.parent.Segment_1238.F0_1238_001e_ShowDialog(
												ClassicGameText.Current.Format(
													ClassicGameTextKey.DiplomatsReportCapitalMoved,
													nationality,
													cityName),
												80,
												80);
										...
										else
										{
											...
											this.parent.Segment_1238.F0_1238_001e_ShowDialog(
												ClassicGameText.Current.Format(
													ClassicGameTextKey.CapitalMoved,
													nationality,
													cityName),
												80,
												80);
				...
				L2307:
					if ((this.parent.GameData.PlayerFlags & (1 << this.Var_6548_PlayerID)) == 0)
					{
						...
						if ((city.StatusFlag & 0x1) != 0 && city.CurrentProductionID < 0 && city.ShieldsCount != 0)
						{
							...
							local_cc = this.parent.Tools.F0_2dc4_007c_CheckValueRange(
						...
						if ((this.parent.MapManagement.F0_2aea_14e0_GetCellActiveUnitPlayerID(local_d8, local_e4) == -1 || 
				...
				if (flag == 1)
				{
					...
					this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
						95,
						106,
						128,
						114,
						ClassicGameText.Current[
							ClassicGameTextKey.CityInfoButton],
						9);
					...
					this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
						...
						114,
						ClassicGameText.Current[
							ClassicGameTextKey.CityHappyButton],
						9);
					...
					this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
						...
						114,
						ClassicGameText.Current[
							ClassicGameTextKey.CityMapButton],
						9);
					...
					this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
						...
						114,
						ClassicGameText.Current[
							ClassicGameTextKey.CityViewButton],
						9);
				...
				this.parent.Var_70da_Arr[3] = this.parent.Tools.F0_2dc4_007c_CheckValueRange(
				...
				this.parent.Var_e17a = this.parent.Tools.F0_2dc4_007c_CheckValueRange(
				...
				if (this.parent.GameData.Players[this.Var_6548_PlayerID].GovernmentType < 4)
				{
					...
					this.parent.Var_70e4 -= this.parent.Tools.F0_2dc4_007c_CheckValueRange(local_e6, 0, this.parent.Var_70e4);
				}
				else
				{
					// Instruction address 0x1d12:0x3090, size: 5
					local_e8 = F0_1d12_6c97_PlayerHasWonder(this.Var_6548_PlayerID, WonderEnum.WomensSuffrage) ? 0 : 1;
				...
				if (flag == 1 && this.Var_2496 == 1 && (this.parent.Var_70e2 - this.parent.Var_70e4) != local_e8)
				{
					...
					this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0(
						ClassicGameText.Current[ClassicGameTextKey.Wonders],
						190, local_42 + 5, 15);
			...
			L32e0:
				...
				this.parent.Var_deb8 = 0;
				this.parent.Var_d2f6 = 0;
				int project1991EffectiveSupportSize =
					city.ActualSize +
					project1991SupportSizeGrace;
				...
				if (project1991EffectiveSupportSize <
					this.parent.Var_deb8)
				{
					this.parent.Var_d2f6 =
						this.parent.Var_deb8 -
						project1991EffectiveSupportSize;
				}
				...
				for (int i = 0; i < 128; i++)
				{
						...
						this.parent.GameData.Players[this.Var_6548_PlayerID].Units[i].HomeCityID == cityID)
					{
						if (this.parent.GameData.Players[this.Var_6548_PlayerID].Units[i].UnitType != UnitTypeEnum.Diplomat &&
							this.parent.GameData.Players[this.Var_6548_PlayerID].Units[i].UnitType != UnitTypeEnum.Caravan &&
							i != project1991NewSupportedUnitGraceID)
						{
							this.parent.Var_deb8++;
								...
								this.parent.GameData.Players[this.Var_6548_PlayerID].GovernmentType >= 4))
							{
								...
								for (int j = 0; j < 128; j++)
								{
									if (this.parent.GameData.Players[this.Var_6548_PlayerID].Units[j].UnitType != UnitTypeEnum.None &&
										...
										j != project1991NewSupportedUnitGraceID)
									{
										// Instruction address 0x1d12:0x3513, size: 5
										local_b2 = this.parent.Tools.F0_2dc4_0289_GetShortestDistance(
											city.Position.X,
											city.Position.Y,
								...
								if (this.parent.Var_70da_Arr[1] >= this.parent.Var_d2f6)
								{
										...
										this.parent.GameData.Players[this.parent.GameData.HumanPlayerID].DiscoveredTechnologyCount)
									{
										...
										this.parent.GameData.Players[this.Var_6548_PlayerID].Units[local_108].UnitType != UnitTypeEnum.Settler)
									{
								...
								else
								{
									if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
									{
										...
										UnitTypeEnum unsupportedUnitType =
											this.parent.GameData.Players[this.Var_6548_PlayerID].Units[local_108].UnitType;
										this.parent.Segment_1238.F0_1238_001e_ShowDialog(
											ClassicGameText.Current.Format(
												ClassicGameTextKey.CityCannotSupportUnit,
												this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID),
												ClassicDisplayNames.Unit(
													unsupportedUnitType,
													this.parent.GameData.Units[(int)unsupportedUnitType].Name)),
											100,
											80);
									}
				...
				if (flag == 0)
				{
					...
					this.parent.Var_e3c2 += this.parent.Tools.F0_2dc4_007c_CheckValueRange(this.parent.Var_deb8, 0, city.ActualSize);
				...
				if (flag == 1)
				{
					...
					if (local_da == 0)
					{
						for (int i = 0; i < 21; i++)
						{
							if (this.parent.GameData.WonderCityID[i + 1] == cityID)
							{
								...
								this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0(
									this.parent.LanguageTools.F0_2f4d_04f7_TrimStringToWidth(
										ClassicDisplayNames.Wonder(
											(WonderEnum)(i + 1),
											this.parent.GameData.Wonders[i + 1].Name)
											.ToUpper(),
										63),
									253,
									local_e4 + 2,
					...
					for (int i = local_da; i < 24; i++)
					{
						if (city.HasImprovement(City.BitToImprovementEnum(i)))
						{
							...
							else
							{
								...
								this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0(
									this.parent.LanguageTools.F0_2f4d_04f7_TrimStringToWidth(
										ClassicDisplayNames.Improvement(
											(ImprovementEnum)(i + 1),
											this.parent.GameData.GetImprovementType(i + 1).Name),
										56),
									253,
									local_e4 + 2,
									15);
					...
					if ((local_ca & 2) != 0)
					{
						...
						this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
							...
							315,
							96,
							ClassicGameText.Current[ClassicGameTextKey.More],
							9);
					}
					...
					local_44 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(((local_de - 1) / 10) + 1, ((city.ShieldsCount - 1) / 100) + 1, 99);
					...
					if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
					{
						...
						this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
							...
							263,
							114,
							(city.StatusFlag & 0x10) != 0
								? ClassicGameText.Current[ClassicGameTextKey.AutomaticProduction]
								: ClassicGameText.Current[ClassicGameTextKey.ChangeProduction],
							9);
						...
						this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
							...
							311,
							114,
							ClassicGameText.Current[ClassicGameTextKey.BuyProduction],
							9);
					}
					...
					else
					{
						...
						this.parent.DrawTools.F0_1182_00b3_DrawCenteredStringToScreen0(
							this.parent.LanguageTools.F0_2f4d_04f7_TrimStringToWidth(
								this.GetLocalizedImprovementTypeName(
									-city.CurrentProductionID),
								86),
							274,
							100,
					...
					if (city.ActualSize != 0)
					{
						local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(80 / city.ActualSize, 1, 8);
					}
					else
					{
						local_e8 = 1;
					...
					this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0(
						ClassicGameText.Current[ClassicGameTextKey.FoodStorage],
						8, 108, 15);
					...
					int iTemp = this.parent.Tools.F0_2dc4_007c_CheckValueRange(city.FoodCount, 0, (city.ActualSize + 1) * local_4a);
					...
					this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0(
						ClassicGameText.Current[ClassicGameTextKey.CityResources],
						8, local_fc, 15);
					...
					for (int i = 0; i < 3; i++)
					{
						...
						if (i == 0)
						{
							...
							local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(
								116 / (this.parent.Tools.F0_2dc4_007c_CheckValueRange(this.parent.Var_70da_Arr[i], 
						...
						else
						{
							...
							local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(116 / (this.parent.Var_70da_Arr[i] + 1), 1, 8);
						}
					...
					if ((city.ActualSize * 2) + (local_48 * this.parent.Var_e3c6) > this.parent.Var_70da_Arr[0])
					{
						...
						local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(116 / ((city.ActualSize * 2) + (local_48 * this.parent.Var_e3c6) + 1), 1, 8);
					...
					if (this.parent.Var_70da_Arr[1] < this.parent.Var_d2f6)
					{
						...
						local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(116 / (this.parent.Var_70da_Arr[1] + 1), 1, 8);
					...
					if (this.parent.Var_d2e0 != 0)
					{
						...
						local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(116 / (this.parent.Var_70da_Arr[2] + 1), 1, 8);
					...
					local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange((city.ActualSize * 8) + 24, 0, 128);
					...
					if (this.Var_2496 == 0)
					{
						...
						local_fc = this.parent.Tools.F0_2dc4_007c_CheckValueRange(
							100 / this.parent.Tools.F0_2dc4_007c_CheckValueRange(local_e0, 1, 99), 1, 8);
					...
					Draw1991PlusCityIntelligence(cityID);
					...
					if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
					{
						...
						this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
							...
							198,
							ClassicGameText.Current[
								ClassicGameTextKey.CityExitButton],
							12);
						...
						this.parent.DrawTools.DrawFilledRectangleWithCenteredText(
							...
							198,
							ClassicGameText.Current[
								ClassicGameTextKey.CityRenameButton],
							9);
						...
						// The desktop presenter polls the primary framebuffer on
						// its own UI thread. This legacy input loop used to spin
						// without yielding when the city had no flashing
						// production warning, starving that presenter and making
						// the already drawn city screen look like a frozen world
						// map. Publish the completed frame and yield while waiting
						// without changing the original input or city rules.
						if (this.parent.Graphics.Screens.ContainsKey(0))
						{
							this.parent.Graphics.Screens
								.GetValueByKey(0).Modified = true;
						}
						this.parent.CityWorkerWaitingForInputForTests?.Invoke(cityID);
					...
					L4bf7:
						if (city.CurrentProductionID >= 0)
						{
							...
							local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange((10 * local_106) - city.ShieldsCount, 0, 999);
						...
						else
						{
							...
							local_e8 = this.parent.Tools.F0_2dc4_007c_CheckValueRange((10 * local_106) - city.ShieldsCount, 0, 999) * 2;
						...
						string completeProductionText =
							ClassicGameText.Current.Format(
								ClassicGameTextKey.CityProductionCost,
								this.GetLocalizedProductionName(
									city.CurrentProductionID),
								local_e8,
								this.parent.GameData.Players[
									this.Var_6548_PlayerID].Coins);

						if ((city.StatusFlag & 0x1) != 0)
						{
							completeProductionText +=
								ClassicGameText.Current[
									ClassicGameTextKey.CityProductionCivilDisorder];
						}
						else if (this.parent.GameData.Players[this.Var_6548_PlayerID].Coins >= local_e8)
						{
							completeProductionText +=
								ClassicGameText.Current[
									ClassicGameTextKey.CityProductionPurchasePrompt];
						}
					...
					L4fff:
						for (local_106 = 0; local_106 < local_4e && (this.parent.GameData.Players[this.Var_6548_PlayerID].Units[Arr_74[local_106]].Status & 
							(UnitStatusEnum.Sentry | UnitStatusEnum.SettlerBuildRoadOrRail | UnitStatusEnum.Fortifying | UnitStatusEnum.Fortified |
								UnitStatusEnum.SettlerBuildIrrigation | UnitStatusEnum.SettlerBuildMineOrForest)) == UnitStatusEnum.None; local_106++)
						{
						}
					...
					L504b:
						...
						local_106 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(local_ea, 0, local_4e - 1);

						if (local_fc == '\n' || local_fc == ' ')
						{
							this.parent.GameData.Players[this.Var_6548_PlayerID].Units[Arr_74[local_106]].ClearStatusFlags(
								UnitStatusEnum.Sentry | UnitStatusEnum.SettlerBuildRoadOrRail | UnitStatusEnum.Fortifying | UnitStatusEnum.Fortified | 
									UnitStatusEnum.SettlerBuildIrrigation | UnitStatusEnum.SettlerBuildMineOrForest);
							this.parent.GameData.Players[this.Var_6548_PlayerID].Units[Arr_74[local_106]].GoToDestination = OpenCivOneGame.InvalidPosition;
						}
					...
					L52f3:
						...
						local_106 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(local_ea, 0, local_b4 - 1);
					...
					L53ad:
						this.parent.MenuBoxDialog.F0_2d05_0031_ShowMenuBox(
							ClassicGameText.Current[
								ClassicGameTextKey.CitySpecialistsRequireFivePopulation],
							32,
							32,
							true,
							false,
					...
					L55b6:
						if ((this.parent.GameData.MapVisibility[
							this.parent.MapManagement.AdjustXPosition(
								city.Position.X + local_b6),
							city.Position.Y + local_be] &
							(1 << this.parent.GameData.HumanPlayerID)) == 0) goto L55f5;
						goto L5605;
					...
					L57a5:
						...
						if (this.parent.MenuBoxDialog.F0_2d05_0031_ShowMenuBox(
							ClassicGameText.Current.Format(
								ClassicGameTextKey.CitySellImprovementPrompt,
								ClassicDisplayNames.Improvement(
									(ImprovementEnum)(local_bc + 1),
									this.parent.GameData.GetImprovementType(
										local_bc + 1).Name),
								10 * this.parent.GameData.GetImprovementType(
									local_bc + 1).Cost),
							128,
							80,
							true,
					...
					L598a:
						...
						this.parent.GameData.Players[this.Var_6548_PlayerID].Units[Arr_74[local_106]].ClearStatusFlags(UnitStatusEnum.Sentry |
							UnitStatusEnum.SettlerBuildRoadOrRail | UnitStatusEnum.Fortifying | UnitStatusEnum.Fortified | UnitStatusEnum.SettlerBuildIrrigation |
					...
					L5aa6:
						...
						if ((this.parent.GameData.MapVisibility[
							this.parent.MapManagement.AdjustXPosition(
								city.Position.X +
									this.parent.CityOffsets[local_ea].X),
							city.Position.Y +
								this.parent.CityOffsets[local_ea].Y] &
				...
				// Reports and 1991+ forecasts need the fully processed resource
				// and happiness scratch values, but must stop before every
				// turn-end mutation below (disorder transitions, treasury,
				// research, pollution, maintenance and fortified-unit packing).
				if (flag == MeasurementOnlyFlag)
				{
					return this.parent.Var_70e2 -
						this.parent.Var_70e4;
				}
				...
				if (this.parent.GameData.GameSettingFlags.Animations && (city.StatusFlag & 0x1) == 0 && this.parent.Var_6b92 == 0)
				{
					this.parent.CityView.F19_0000_0000_ShowCityLayout(cityID, -2,
						ClassicGameText.Current.Format(
							ClassicGameTextKey.CityCivilDisorder,
							this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
			...
			L5eb4:
				if (this.parent.GameData.Players[this.Var_6548_PlayerID].GovernmentType == 5)
				{
					if (this.Var_6548_PlayerID == this.parent.GameData.HumanPlayerID)
					{
						...
						this.parent.News.F21_0000_0000_ShowNews(cityID,
							ClassicGameText.Current.Format(
								ClassicGameTextKey.CityGovernmentCollapsed,
								this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
			...
			L5f7a:
				...
				this.parent.Segment_1238.F0_1238_001e_ShowDialog(
					ClassicGameText.Current.Format(
						ClassicGameTextKey.CityOrderRestored,
						this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)),
					100,
					80);
			...
			L60e8:
				...
				if (this.parent.GameData.GameSettingFlags.Animations)
				{
					this.parent.CityView.F19_0000_0000_ShowCityLayout(cityID, -2,
						ClassicGameText.Current.Format(
							ClassicGameTextKey.CityLeaderDayCelebrated,
							GetLocalizedRulerTitle(
								this.parent.GameData.Players[
									this.Var_6548_PlayerID].GovernmentType),
							this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
			...
			L6251:
				...
				this.parent.News.F21_0000_0000_ShowNews(cityID,
					ClassicGameText.Current.Format(
						ClassicGameTextKey.CityLeaderCelebrationCanceled,
						GetLocalizedRulerTitle(
							this.parent.GameData.Players[
								this.Var_6548_PlayerID].GovernmentType),
						this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
			...
			L64b8:
				...
				if (ShouldAddPollution(
					local_e0,
					this.parent.GameData.Players[this.Var_6548_PlayerID].DiscoveredTechnologyCount,
					this.parent.GameData.DifficultyLevel,
					this.parent.CAPI.RNG)) goto L6545;
				goto L6640;
			...
			L65bf:
				...
				this.parent.Segment_1238.F0_1238_001e_ShowDialog(
					ClassicGameText.Current.Format(
						ClassicGameTextKey.CityPollutionNear,
						this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)),
					80,
					64);
			...
			L6691:
				...
				this.parent.Segment_29f3.F0_29f3_0ec3_AddEffectsOfNuclearAttackOrCatastrophe(local_d8, local_e4);
				...
				this.parent.News.F21_0000_0000_ShowNews(cityID,
					ClassicGameText.Current.Format(
						ClassicGameTextKey.CityNuclearCatastrophe,
						this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID)));
			...
			L681b:
				...
				this.parent.Segment_1238.F0_1238_001e_ShowDialog(
					ClassicGameText.Current.Format(
						ClassicGameTextKey.CityCannotMaintainImprovement,
						this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityID),
						ClassicDisplayNames.Improvement(
							(ImprovementEnum)(local_bc + 1),
							this.parent.GameData.GetImprovementType(
		...
		internal void DrawCityNeighborhood(City city, int[,] unavailableCells)
		{
			...
			for (int i = 0; i < this.Var_6540_CityOffsetCount; i++)
			{
				int x =
					city.Position.X +
					this.parent.CityOffsets[i].X;
				int y =
					city.Position.Y +
					this.parent.CityOffsets[i].Y;

				if (city.PlayerID == this.parent.GameData.HumanPlayerID &&
					(this.parent.GameData.MapVisibility[
						this.parent.MapManagement.AdjustXPosition(x),
						y] & (1 << city.PlayerID)) == 0)
				{
					continue;
				}
				...
				int activeUnitPlayerID =
					this.parent.MapManagement
						.F0_2aea_14e0_GetCellActiveUnitPlayerID(x, y);
				bool hasEnemyUnit =
					activeUnitPlayerID != -1 &&
					activeUnitPlayerID != city.PlayerID;

				if (hasEnemyUnit)
				{
					// DrawCellWithUnit deliberately applies the human unit-
					// visibility mask. The red city-neighborhood border is
					// still required for every occupied enemy square, including
					// a unit whose sprite is currently invisible.
				...
				else
				{
					...
					this.parent.MapManagement.F0_2aea_03ba_DrawCell(x, y);
				}

				if (hasEnemyUnit ||
					unavailableCells[
						this.parent.CityOffsets[i].X + 2,
						this.parent.CityOffsets[i].Y + 2] != 0)
				{
					// Instruction address 0x1d12:0x06f6, size: 5
					this.parent.DrawTools.DrawRectangle(
						(x - this.parent.Var_d4cc_MapViewX) * 16 + 80,
						(y - this.parent.Var_d75e_MapViewY) * 16 + 8,
						15,
						15,
						12);
				}
		...
		public int F0_1d12_6abc_GetCityResourceCount(int playerID, int cityID, int x, int y, CityResourceTypeEnum resourceType)
		{
			...
			if (this.parent.MapManagement.ValidateMapCoordinates(x, y))
			{
		...
		public bool F0_1d12_6c97_PlayerHasWonder(int playerID, WonderEnum wonderType)
		{
			...
			int wonderCityID =
				this.parent.GameData.WonderCityID[(int)wonderType];

			if (i < 8 ||
				wonderCityID < 0 ||
				wonderCityID >= this.parent.GameData.Cities.Length ||
				this.parent.GameData.Cities[wonderCityID].StatusFlag == 0xff ||
				this.parent.GameData.Cities[wonderCityID].PlayerID != playerID)
			{
				return false;
			}
			else
		...
		private int F0_1d12_6cf3_GetWonderCityID(int wonderID)
		{
			...
			int wonderCityID = this.parent.GameData.WonderCityID[wonderID];
		...
		internal static int GetPollutionRandomUpperBound(
			short discoveredTechnologyCount,
			short difficultyLevel)
		{
			// The original formula is
			// Random(256 - advances * difficulty / 2). Its missing absolute
			// value is the documented late-Future-Tech pollution bug: a
			// negative random range can make even a clean city pollute.
			// Keep the signed-word save fields and the Classic coefficients,
			// but apply the established DOS executable patch at the arithmetic
			// boundary before asking the RNG for a value.
			return Math.Abs(
				256 -
				((discoveredTechnologyCount * difficultyLevel) / 2));
		}

		internal static bool ShouldAddPollution(
			int cityPollution,
			short discoveredTechnologyCount,
			short difficultyLevel,
			RandomMT19937 random)
		{
			return (cityPollution * 2) >
				random.Next(
					GetPollutionRandomUpperBound(
						discoveredTechnologyCount,
						difficultyLevel));
		}
		...
		private void F0_1d12_6dfe(int cityID, int size)
		{
			...
			this.parent.Var_70e2 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(this.parent.Var_70e2, 0, this.parent.GameData.Cities[cityID].ActualSize);
			// Instruction address 0x1d12:0x6e98, size: 5
			this.parent.Var_70e4 = this.parent.Tools.F0_2dc4_007c_CheckValueRange(this.parent.Var_70e4, 0, this.parent.GameData.Cities[cityID].ActualSize);

			// Instruction address 0x1d12:0x6ebb, size: 5
			while ((this.parent.Var_70e2 + this.parent.Var_70e4) > this.parent.Tools.F0_2dc4_007c_CheckValueRange(this.parent.GameData.Cities[cityID].ActualSize - size, 0, 99))
			{
				if (this.Var_6542 > 0)
				{
					this.Var_6542--;
