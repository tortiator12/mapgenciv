	...
	public class CommonTools
	{
		...
		public void TransformPalette(int speed, ushort palettePtr)
		{
			...
			lock (VCPU.GraphicsLock)
			{
				...
				if (!ShouldAnimatePaletteTransitions())
				{
					CompletePaletteTransformImmediately();
					return;
				}
		...
		private bool ShouldAnimatePaletteTransitions() =>
			this.parent.GameData.GameSettingFlags.Animations &&
			this.timer != null;

		private void CompletePaletteTransformImmediately()
		{
			for (int i = 0; i < this.transformColors.Length; i++)
			{
				this.graphics.SetPaletteColor(
					(byte)i,
					this.transformColors[i].ToHSV.ToColor());
			}

			this.transformCount = this.transformValue;
			this.transformFlag = false;
		}
		...
		public void InitSoundEngine()
		{
			...
			this.parent.Var_1a30_SoundDriverType =
				this.parent.Sound.DriverType;
		...
		{
			// The translated game also uses this command to reset its legacy
			// intro/menu timing counter. NSound gates only the audible sink.
			// Instruction address 0x1000:0x0a32, size: 5
			this.parent.Sound.F0_0000_0062_PlayTune(tune, param2);
		}

		/// <summary>
		/// Changes the sound option and always stops the platform sink when
		/// sound is disabled.
		/// </summary>
		/// <param name="enabled">Whether future tune commands may play.</param>
		public void SetSoundEnabled(bool enabled)
		{
			this.parent.GameData.GameSettingFlags.Sound = enabled;
			if (!enabled)
			{
				this.parent.Sound.Stop();
			}
		}
		...
		public void SetMousePositionAndIcon(int x, int y, int bitmapPtr)
		{
			...
			// Original cursor handles are unsigned 16-bit DOS resource
			// pointers. Some faithfully ported call sites still pass them
			// through a signed short, so preserve the underlying word.
			this.parent.Var_5876_MouseIcon = bitmapPtr < 0
				? unchecked((ushort)bitmapPtr)
				: bitmapPtr;
		}
		...
		/// <summary>
		/// Clear keyboard and mouse events and ensure no mouse button is pressed
		/// </summary>
		public void ClearKeyboardAndMouseEvents()
		{
			this.CPU.DoEvents();
			...
			this.parent.ClearMouseEventsAndEnsureNoMouseButtonIsPressed();
		}

		/// <summary>
		/// Clear keyboard events and ensure no mouse button is pressed
		/// </summary>
		public void ClearKeyboardEventsAndEnsureNoMouseButtonIsPressed()
		{
			this.CPU.DoEvents();
			...
			while (this.parent.GetMouseEvent().Buttons != MouseButtonsEnum.None) { Thread.Sleep(1); }
		}
	}
}
