...
namespace OpenCivOne
{
	public class ShowDebugDetails
	{
			...
			for (int i = 0; i < 50; i++)
			{
				for (int j = 0; j < 80; j++)
				{
					if ((this.oParent.GameData.MapVisibility[j, i] & playerBitMask) != 0 || this.oParent.Var_d806_DebugFlag)
					{
						...
						{
							int unitPlayerID = this.oParent.MapManagement.F0_2aea_14e0_GetCellActiveUnitPlayerID(j, i);
		...
		public void ShowPlayerDetails(int playerID)
		{
			...
			this.oParent.DrawTools.F0_1182_005c_DrawStringToScreen0(
				$"Population: {this.oParent.Tools.F0_2dc4_0337_PopulationValueToString(this.oParent.Tools.F0_2dc4_02cd_GetPlayerTotalPopulationCount(playerID))}", 16, 32, 15);
