...
namespace OpenCivOne
{
	public class MapManagement
	{
		...
		private readonly Dictionary<GPoint, bool>
			activeHumanGoToPathMarkers = [];

		public MapManagement(OpenCivOneGame parent)
		{
			this.parent = parent;
		...
		{
			GBitmap? earthMap = GBitmap.FromPICFile(
				this.parent.RuntimeOptions.GetResourceFilePath("MAP.PIC"),
				true);
		...
		public void F0_2aea_0008_DrawVisibleMap(int playerID, int x, int y)
		{
			...
			this.parent.Var_d75e_MapViewY = this.parent.Tools.F0_2dc4_007c_CheckValueRange(y, 0, 38);
			...
			RebuildActiveHumanGoToPathMarkerCache();

			// Draw Visible Map on screen

			/*int cellDrawOrder = this.parent.CAPI.RNG.Next(256);

			for (int i = 0; i < 256; i++)
			{
				int cellXPos = (cellDrawOrder & 0xf);
				int cellYPos = (cellDrawOrder & 0xf0) >> 4;

				if (cellXPos < 15 && cellYPos < 12)
				{
					if (this.parent.Var_d806_DebugFlag ||
						(this.parent.GameData.MapVisibility[this.parent.MapManagement.AdjustXPosition(this.parent.Var_d4cc_MapViewX + cellXPos),
							this.parent.Var_d75e_MapViewY + cellYPos] & mapPlayerVisibilityMask) != 0)
					{
						// Instruction address 0x2aea:0x0122, size: 3
						F0_2aea_11d4_DrawCellWithUnit(this.parent.MapManagement.AdjustXPosition(this.parent.Var_d4cc_MapViewX + cellXPos), this.parent.Var_d75e_MapViewY + cellYPos);
					}
					else
					{
						// Instruction address 0x2aea:0x0094, size: 5
						this.parent.DrawTools.FillRectangle(this.parent.Var_aa_Screen0_Rectangle, 80 + (cellXPos * 16), 8 + (cellYPos * 16), 16, 16, 0);
					}
				}
			...
			for (int i = 0; i < 15; i++)
			{
				for (int j = 0; j < 12; j++)
				{
					int mapX = this.parent.MapManagement.AdjustXPosition(this.parent.Var_d4cc_MapViewX + i);
					int mapY = this.parent.Var_d75e_MapViewY + j;

					if (this.parent.Var_d806_DebugFlag || (this.parent.GameData.MapVisibility[mapX, mapY] & mapPlayerVisibilityMask) != 0)
					{
						// Instruction address 0x2aea:0x0122, size: 3
					...
					{
						// Instruction address 0x2aea:0x0094, size: 5
						this.parent.DrawTools.FillRectangle(this.parent.Var_aa_Screen0_Rectangle, 80 + (i * 16), 8 + (j * 16), 16, 16, 0);
					}
				}
			}
			...
			for (int i = 0; i < this.VisibleCities.Count; i++)
			{
				...
				this.parent.DrawTools.F0_1182_0086_DrawStringWithShadow(
					this.parent.LanguageTools.F0_2f4d_04f7_TrimStringToWidth(this.parent.Segment_2459.F0_2459_08c6_GetCityName(cityInfo.CityID), 327 - cityInfo.ScreenPos.X),
					this.parent.Tools.F0_2dc4_007c_CheckValueRange(cityInfo.ScreenPos.X - 8, 80, 999), cityInfo.ScreenPos.Y + 16, 11);
			}
			...
			if (this.parent.Segment_1ade.F0_1ade_22b5_PlayerHasTechnology(playerID, TechnologyAdvanceEnum.Navigation))
			{
				yMap = 0;
			}
			...
			this.parent.Var_6ed6_MiniMapX = xMap;
			this.parent.Var_70ea_MiniMapY = yMap;

			// Draw Mini Map

			// Instruction address 0x2aea:0x0233, size: 5
			this.parent.DrawTools.FillRectangle(this.parent.Var_aa_Screen0_Rectangle, 0, 8, 80, 50, 0);
			...
			for (int i = 0; i < 128; i++)
			{
				...
				if (city.StatusFlag != 0xff && (city.VisibleSize != 0 || city.PlayerID == this.parent.GameData.HumanPlayerID))
				{
					...
					int cellXPos = this.parent.MapManagement.AdjustXPosition(city.Position.X - mapXSrc);
					int cellYPos = city.Position.Y - mapYSrc + mapYOffset;
					...
					if (cellYPos >= 0 && cellYPos < 50)
					{
						...
						this.parent.Graphics.F0_VGA_0550_SetPixel(this.parent.Var_aa_Screen0_Rectangle.ScreenID, cellXPos, cellYPos + 8,
							this.parent.Array_1946_PlayerColours[city.PlayerID]);
					}
				}
			}

			// Instruction address 0x2aea:0x037c, size: 5
			this.parent.DrawTools.DrawShadowRectangle(0, 8, 79, 49, 15, 8);

			// Instruction address 0x2aea:0x03a2, size: 5
			this.parent.DrawTools.DrawRectangle(x - xMap - 1, y - yMap + 7, 17, 10, 15);
		}
		...
		public bool F0_2aea_03ba_DrawCell(int x, int y)
		{
			...
			else
			{
				...
				if (terrainType == TerrainTypeEnum.Water)
				{
					...
					for (int i = 1; i < 9; i++)
					{
						...
						if (GetTerrainType(this.parent.MapManagement.AdjustXPosition(x + direction.X), y + direction.Y) != TerrainTypeEnum.Water &&
							ValidateMapCoordinates(0, y + direction.Y))
						{
							mask |= 0x80;
						}
					}
					...
					for (int i = 0; i < 4; i++)
					{
						if (i < 2)
						{
							...
							this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX + ((i & 0x1) * 8), scrY,
								this.parent.Array_d294[(bitmapMask >> (i * 2)) & 0x7, i]);
						}
						else
						{
							// Instruction address 0x2aea:0x05f4, size: 5
							this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX - ((i & 0x1) * 8) + 8, scrY + 8,
								this.parent.Array_d294[(bitmapMask >> (i * 2)) & 0x7, i]);
						}
					}
					...
					for (int i = 1; i < 9; i += 2)
					{
						...
						if (GetTerrainType(this.parent.MapManagement.AdjustXPosition(x + direction.X), y + direction.Y) == TerrainTypeEnum.River)
						{
							...
							this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d2d4[(i - 1) / 2]);
						}
					}
				}
					...
					this.parent.Var_dcfc == 0 && !terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.City))
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d4ce[4]);
				}
				...
				if (terrainType == TerrainTypeEnum.River)
				{
					...
					if (mask > 0)
					{
						...
						this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_6e00[mask - 1]);
					}
				}
				...
				if (terrainType != TerrainTypeEnum.Water && terrainType != TerrainTypeEnum.River)
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_b886[(int)terrainType, mask]);
					...
					if (terrainType == TerrainTypeEnum.Grassland && (((7 * x) + (11 * y)) & 0x2) == 0)
					{
						...
						this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX + 4, scrY + 4, this.parent.Var_b880);
					}
				}
				...
				if (terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.Pollution))
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d4ce[6]);
				}
				...
				if (terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.Road))
				{
					...
					for (int i = 1; i < 9; i++)
					{
						...
						if (terrainImprovements1.HasFlag(TerrainImprovementFlagsEnum.Road))
						{
							...
							if ((terrainImprovements & railRoadOrCity) == 0 || (terrainImprovements1 & railRoadOrCity) == 0)
							{
								...
								this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_b27a[i - 1]);
							}
							else
							{
								// Instruction address 0x2aea:0x0a73, size: 5
								this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_b29a[i - 1]);
							}
						}
					}
					...
					if (roadIcon != -1)
					{
						...
						this.parent.DrawTools.FillRectangle(this.parent.Var_aa_Screen0_Rectangle, scrX + 7, scrY + 7, 2, 2, (ushort)roadIcon);
					}
				}
				...
				if (terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.Mines) && this.parent.Var_dcfc == 0)
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d4ce[5]);
				}
				...
				if (F0_2aea_1836_CellHasSpecialResource(x, y))
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d4ce[(int)terrainType + 16]);
				}
				...
				if (F0_2aea_1894_CellHasMinorTribeHut(x, y, terrainType))
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d4ce[31]);
				}
				...
				if (terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.Fortress))
				{
					...
					this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_d4ce[30]);
				}
				...
				if (!this.parent.Var_d806_DebugFlag)
				{
					...
					for (int i = 1; i < 9; i += 2)
					{
						...
						if ((visibilityMask & (0x1 << this.parent.GameData.HumanPlayerID)) == 0)
						{
							...
							this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX, scrY, this.parent.Array_7eec[(i - 1) / 2]);
						}
					}
				}
				...
				if (terrainImprovements.HasFlag(TerrainImprovementFlagsEnum.City) && this.parent.Var_dcfc == 0)
				{
					...
					int cityID = this.parent.Tools.F0_2dc4_00ba_GetCityByLocation(x, y);
					...
					if (cityID != -1)
					{
						...
						if (city.PlayerID == this.parent.GameData.HumanPlayerID || city.VisibleSize > 0 || this.parent.Var_d806_DebugFlag)
						{
							...
							this.parent.DrawTools.DrawRectangle(scrX + 1, scrY + 1, 13, 13, 15);

							// Draw city dark borders at the top and right sides

							// Instruction address 0x2aea:0x0c7d, size: 5
							this.parent.DrawTools.DrawRectangle(scrX + 2, scrY + 1, 12, 12, this.parent.Array_1956[city.PlayerID]);

							// Draw city main color

							// Instruction address 0x2aea:0x0cac, size: 5
							this.parent.DrawTools.FillRectangle(this.parent.Var_aa_Screen0_Rectangle, scrX + 2, scrY + 2, 12, 12,
								this.parent.Array_1946_PlayerColours[city.PlayerID]);

							// Draw city 'streets'

							// Instruction address 0x2aea:0x0cba, size: 5
							this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX + 1, scrY + 1, this.parent.Array_d4ce[28]);
							...
							if ((city.StatusFlag & 0x1) != 0)
							{
								...
								this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX + 5, scrY + 2, this.parent.Array_6e96[4]);
							}
							else
							{
								// Draw city size
								...
								this.parent.DrawTools.F0_1182_005c_DrawStringToScreen0($"{citySize}", ((citySize < 10) ? 6 : 3) + scrX, scrY + 5, 0);
							}
							...
							if (F0_2aea_1458_GetCellActiveUnitID(x, y) != -1 || city.Unknown[0] != -1)
							{
								...
								this.parent.DrawTools.DrawRectangle(scrX, scrY, 15, 15, 0);
							}

							if (city.HasImprovement(ImprovementEnum.CityWalls))
							{
								// Draw city walls

								// Instruction address 0x2aea:0x0de7, size: 5
								this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, scrX + 1, scrY + 1, this.parent.Array_d4ce[29]);
							}
		...
		public bool F0_2aea_0e29_DrawUnit(int playerID, int unitID)
		{
			...
			{
				if ((unit.Status & UnitStatusEnum.Sentry) == UnitStatusEnum.None || GetTerrainType(unit.Position.X, unit.Position.Y) != TerrainTypeEnum.Water ||
					this.parent.GameData.Units[(int)unit.UnitType].MovementType == UnitMovementTypeEnum.Water)
				{
					if (unit.NextUnitID != -1)
					{
						...
						this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, x + 1, y + 1,
							this.parent.Array_d4ce[64 + (playerID * 32) + (int)unit.UnitType]);
					}
		...
		public void F0_2aea_0fb3_DrawUnitWithStatus(int playerID, int unitID, int x, int y)
		{
			...
			this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, x, y,
				this.parent.Array_d4ce[64 + (playerID * 32) + (int)unit.UnitType]);

			if ((unit.Status & UnitStatusEnum.Fortified) != UnitStatusEnum.None)
			{

				// Instruction address 0x2aea:0x0ffb, size: 5
				this.parent.Graphics.F0_VGA_0c3e_DrawBitmapToScreen(this.parent.Var_aa_Screen0_Rectangle, x, y, this.parent.Array_d4ce[29]);
			}
			else
			{
				if ((unit.Status & UnitStatusEnum.Fortifying) != UnitStatusEnum.None)
				{
					// Instruction address 0x2aea:0x103d, size: 5
					this.parent.DrawTools.F0_1182_0086_DrawStringWithShadowToScreen0("F", x + 4, y + 7, (byte)((playerID == 1) ? 9 : 15));
				}
			}
			...
			if (playerID == this.parent.GameData.HumanPlayerID && unit.GoToDestination.X != -1)
			{
				...
				this.parent.DrawTools.F0_1182_0086_DrawStringWithShadowToScreen0("G", x + 4, y + 7, (byte)((playerID == 1) ? 9 : 15));
			}

			if ((unit.Status & (UnitStatusEnum.SettlerBuildRoadOrRail | UnitStatusEnum.SettlerBuildIrrigation | UnitStatusEnum.SettlerBuildMineOrForest)) != UnitStatusEnum.None && 
				this.parent.GameData.Units[(int)unit.UnitType].MovementType != UnitMovementTypeEnum.Air)
			{
				string status = "R";

				if ((unit.Status & UnitStatusEnum.SettlerBuildIrrigation) != UnitStatusEnum.None)
				{
					status = (unit.UnitType != UnitTypeEnum.Settler) ? "?" : "I";
				}

				if ((unit.Status & UnitStatusEnum.SettlerBuildMineOrForest) != UnitStatusEnum.None)
				{
					status = "M";
					...
					if ((unit.Status & UnitStatusEnum.SettlerBuildRoadOrRail) != UnitStatusEnum.None)
					{
						status = "P";
					}
				}
				...
				this.parent.DrawTools.F0_1182_0086_DrawStringWithShadowToScreen0(status, x + 4, y + 7, (byte)((playerID == 1) ? 9 : 15));

				// Instruction address 0x2aea:0x1172, size: 5
				this.parent.DrawTools.DrawRectangle(x - 1, y - 1, 15, 15, 7);
			}
		
			if ((unit.Status & UnitStatusEnum.Sentry) != UnitStatusEnum.None)
			{
		...
		public void F0_2aea_11d4_DrawCellWithUnit(int x, int y)
		{
			...
			DrawSelectedHumanGoToPathMarker(x, y);
			...
			if (this.parent.Var_dcfc == 0)
			{
				...
				if (unitID != -1)
				{
					int playerID = F0_2aea_14e0_GetCellActiveUnitPlayerID(x, y);
		...
		private void DrawSelectedHumanGoToPathMarker(int x, int y)
		{
			GPoint position = new(AdjustXPosition(x), y);
			if (!this.activeHumanGoToPathMarkers.TryGetValue(
					position,
			...
			int scrX =
				...
				80;
			int scrY =
				(y - this.parent.Var_d75e_MapViewY) *
				16 +
				8;
			if (scrX < 80 || scrX >= 320 ||
				scrY < 8 || scrY > 192)
			...
			if (isDestination)
			{
				// Four small corners keep a city sprite legible while still
				// making the destination distinct from intermediate dots.
		...
		private void RebuildActiveHumanGoToPathMarkerCache()
		{
			this.activeHumanGoToPathMarkers.Clear();
			IReadOnlyList<ClassicGoToPathCellMarker> markers =
				this.parent.ActiveHumanGoToPreviewUnitID >= 0
					? ClassicGoToPathOverlayPlanner.BuildPreview(
						...
						this.parent.ActiveHumanGoToPreviewUnitID,
			...
			foreach (ClassicGoToPathCellMarker marker in markers)
			{
				this.activeHumanGoToPathMarkers[
					new GPoint(
						AdjustXPosition(marker.Position.X),
						marker.Position.Y)] =
		...
		private void F0_2aea_125b_DrawWaterUnit(int playerID, int unitID)
		{
				...
				this.parent.GameData.Units[(int)unit.UnitType].MovementType != UnitMovementTypeEnum.Water)
			{
				...
				do
				{
					...
					if (this.parent.GameData.Units[(int)this.parent.GameData.Players[playerID].Units[currentUnitID].UnitType].MovementType == UnitMovementTypeEnum.Water)
					{
		...
		public bool ValidateMapCoordinates(int x, int y)
		{
		...
		/// <param name="y"></param>
		/// <returns></returns>
		public bool ValidateMapCoordinates(GPoint pt)
		{
			//this.oCPU.Log.EnterBlock($"F0_2aea_1326_CheckMapBounds({xPos}, {yPos})");

			// function body
			return ValidateMapCoordinates(pt.X, pt.Y);
		}
		...
		/// <param name="pt"></param>
		/// <returns>terrain ID</returns>
		public TerrainTypeEnum GetTerrainType(GPoint pt)
		{
			//this.oCPU.Log.EnterBlock($"F0_2aea_134a_GetTerrainID({xPos}, {yPos})");
			// function body
			return this.parent.PixelValuesToTerrainTypes[this.gameData.Map.GetPixel(this.AdjustXPosition(pt.X), pt.Y)];
		}
		...
		public void F0_2aea_138c_SetCityOwner(int x, int y, int playerID)
		{
			...
			ushort oldValue = this.gameData.Map.GetPixel(x + 160, y);

			// Instruction address 0x2aea:0x13c0, size: 3
			this.gameData.Map.SetPixel(x + 160, y, (byte)((oldValue & 0x8) | playerID), 0);
		}
		...
		public void F0_2aea_13cb_SetCellPlayerID(int x, int y, int playerID, int unitID)
		{
			...
			if (activeUnitID != -1)
			{
				...
				this.parent.Segment_29f3.F0_29f3_0b66_AddUnitToStack(playerID, unitID, activeUnitID);
			}
		...
		public void F0_2aea_1412_SetCellActivePlayerID(int x, int y, int playerID, int unitID)
		{
			...
			if (this.parent.GameData.Players[playerID].Units[unitID].NextUnitID != -1)
			{
				...
				this.parent.Segment_29f3.F0_29f3_0bc9_RemoveUnitFromStack(playerID, unitID);
			}
			else
			{
		...
		public int F0_2aea_1458_GetCellActiveUnitID(int x, int y)
		{
			...
			if (playerID != -1)
			{
				for (int i = 0; i < 128; i++)
				{
					...
					if (unit.UnitType != UnitTypeEnum.None && unit.Position.X == x && unit.Position.Y == y)
					{
						return i;
					}
				}
		...
		public int F0_2aea_14e0_GetCellActiveUnitPlayerID(int x, int y)
		{
			// function body
		...
		/// <param name="y"></param>
		/// <returns></returns>
		public bool CanIrrigateCell(int x, int y)
		{
			// function body
			if (this.parent.MapManagement.GetTerrainType(x, y) != TerrainTypeEnum.River)
			{
				for (int Local_8 = 1; Local_8 < 9; Local_8 += 2)
				{
					GPoint direction = this.parent.MoveDirections[Local_8];
					int newX = x + direction.X;
					int newY = y + direction.Y;

					TerrainTypeEnum terrainType = this.parent.MapManagement.GetTerrainType(newX, newY);
					TerrainImprovementFlagsEnum terrainImprovements = this.parent.MapManagement.F0_2aea_1585_GetVisibleTerrainImprovements(newX, newY);
		...
		public void F0_2aea_1653_SetTerrainImprovements(int x, int y, TerrainImprovementFlagsEnum improvements)
		{
			...
			if (this.parent.Var_6b90_CurrentPlayer == this.parent.GameData.HumanPlayerID)
			{
		...
		/// <param name="y"></param>
		/// <returns></returns>
		public int F0_2aea_1942_GetGroupID(GPoint pt)
		{
			// Instruction address 0x2aea:0x1953, size: 5
			return F0_2aea_1942_GetGroupID(pt.X, pt.Y);
		}
		...
		{
			this.gameData.Map.SetPixel(80 + this.AdjustXPosition(x), y + 50, (byte)score);
		}
