...
namespace OpenCivOne
{
	public class MenuBoxDialog
	{
		...
		public int F0_2d05_0031_ShowMenuBox(string menuString, int x, int y, bool windowFrame, bool helpOption, bool emptyKeyboardAndMouse)
		{
			...
			this.parent.MenuBoxWaitingForInputForTests?.Invoke(menuString);
			...
			if (this.parent.Var_db38 == 0)
			{
				...
				while (true)
				{
					...
					bool optionResolvedFromCurrentEvent = false;
					...
					{
						if (ClassicMenuPointerSelection.ShouldUpdateHover(
							mouseEvent.Position.X,
							mouseEvent.Position.Y,
							oldMouseX,
							oldMouseY,
							mouseEvent.Buttons != MouseButtonsEnum.None))
						{
							optionResolvedFromCurrentEvent = ClassicMenuPointerSelection.TryGetOptionIndex(
								mouseEvent.Position.Y,
								contentTop,
								lineHeight,
								optionIndexes,
								out int hoveredOptionIndex);

							if (optionResolvedFromCurrentEvent)
							{
					...
					else if (mouseActive)
					{
						if (mouseEvent.Buttons == MouseButtonsEnum.Left)
						{
							...
							if (ClassicMenuPointerSelection.ShouldConfirmSelection(
								mouseEvent.Buttons,
								optionResolvedFromCurrentEvent,
								selectedOptionIndex,
								oldSelectedOptionIndex,
