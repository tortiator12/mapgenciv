...
namespace OpenCivOne
{
	public class CAPI
	{
		...
		public CAPI(OpenCivOneGame parent)
		{
			this.oParent = parent;
		...
		public short kbhit()
		{
			...
			this.oCPU.AX.UInt16 = (ushort)((this.oParent.Keys.Count > 0) ? 0xffff : 0);
		...
		public short getch()
		{
			...
			while (this.oParent.Keys.Count == 0)
			{
				Thread.Sleep(10);
				this.oCPU.DoEvents();
			}
			...
			{
				key = this.oParent.Keys.Dequeue();
				this.oCPU.AX.UInt16 = (ushort)key;
			}

			this.oParent
				.ObserveConsumedKeyForAutomaticEndTurnPause(key);
		...
		public short fopen(string filename, string mode)
		{
			...
			try
			{
				string sPath = this.oParent.RuntimeFiles.Resolve(
					GetDOSFileName(filename.ToUpper()),
					eMode,
					eAccess);
		...
		public ushort strcpy(ushort destinationPtr, string? source)
		{
			if (source == null)
			{
				this.oCPU.WriteUInt8(this.oCPU.DS.UInt16, destinationPtr, 0);
			}
			else
			{
				this.oCPU.WriteString(VCPU.ToLinearAddress(this.oCPU.DS.UInt16, destinationPtr), source);
