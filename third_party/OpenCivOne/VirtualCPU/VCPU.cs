...
namespace IRB.VirtualCPU
{
	public class VCPU
	{
		...
		public VCPU(OpenCivOne.OpenCivOneGame parent, LogWrapper log)
		{
			this.oParent = parent;
			this.oLog = log;
		...
		public OpenCivOne.OpenCivOneGame Parent
		{
		...
		public bool Pause
		{
			get { return Volatile.Read(ref this.bPause); }
			set { Volatile.Write(ref this.bPause, value); }
		}
		...
		public void DoEvents()
		{
			...
			if (this.bEnableInterrupt && this.bEnableTimer && this.bTimerFlag && !this.bInTimer)
			{
					...
					this.oOldMouseButtons != this.oMouseButtons)
				{
					...
					this.oParent.CommonTools.HandleMouseEvent();
		...
		private string ResolveRuntimeFilePath(
			...
			FileAccess access)
		{
			string dosFileName = CAPI.GetDOSFileName(
				fileName.ToUpperInvariant());

			return this.oParent.RuntimeFiles.Resolve(
				dosFileName,
				mode,
				access);
		}
		...
		private void DOSLoadOrExecuteProgram()
		{
			if (this.oAX.LowUInt8 == 3)
			{
				...
				string sPath = ResolveRuntimeFilePath(
					sName,
					FileMode.Open,
					FileAccess.Read);
		...
		{
			// open file
			string sName = CAPI.GetDOSFileName(
				this.ReadString(
					VCPU.ToLinearAddress(
						this.DS.UInt16,
						this.DX.UInt16)).ToUpper());
		...
		private void DosGetCurrentDirectory()
		{
			...
			string resourcePath = Path.TrimEndingDirectorySeparator(
				this.oParent.RuntimeOptions.ResourcePath);
			string pathRoot = Path.GetPathRoot(resourcePath) ?? string.Empty;
			string sTemp = resourcePath[pathRoot.Length..]
		...
		bool DOS_FCBOpen(ushort seg, ushort offset)
		{
			...
			try
			{
				resolvedPath = ResolveRuntimeFilePath(
					shortname,
					FileMode.Open,
					FileAccess.Read);
			}
			...
			for (int i = 0; i < this.aFileHandles.Length; i++)
			{
				...
				{
					handle = i;
					fcb.FileOpen((byte)handle, resolvedPath);
					return true;
				}
			}

			if (!File.Exists(resolvedPath) || this.iFileCount > 255)
				return false;
			file = new FileStream(
				resolvedPath,
				FileMode.Open,
		...
		public class DOS_FCB
		{
			...
			public void FileOpen(byte _fhandle, string resolvedPath)
			{
